...
export default function SignUpPage() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      dots.forEach(d => {
        ...
        ctx.fillStyle = `rgba(88,166,255,${d.o})`; ctx.fill();
      });
      for (let i = 0; i < dots.length; i++) {
        for (let j = i + 1; j < dots.length; j++) {
          ...
          if (dist < 110) {
            ...
            ctx.strokeStyle = `rgba(88,166,255,${0.05 * (1 - dist / 110)})`; ctx.lineWidth = 0.5; ctx.stroke();
          }
        }
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#080c10] relative overflow-hidden py-10">
      ...
      >
        <div className="border border-[#21262d] rounded-sm bg-[#0d1117]/80 backdrop-blur-sm px-8 py-10"
          ...
          <div className="mb-8">
            ...
            <p className="font-mono text-xs text-[#5c7082]">Join thousands of developers</p>
          </div>

          {/* Google */}
          <button
            onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
            className="w-full flex items-center justify-center gap-2.5 border border-[#21262d] hover:border-[#58a6ff]/30 bg-transparent hover:bg-[#58a6ff]/[0.03] transition-all duration-150 text-[#8b949e] hover:text-[#e6edf3] font-mono text-xs uppercase tracking-wider py-3 rounded-sm mb-5"
          >
          ...
          {/* error */}
          {error && (
            <div className="mb-4 px-3 py-2.5 bg-[rgba(248,81,73,0.06)] border border-[#f85149]/40 rounded-sm font-mono text-xs text-[#f85149]">
              ⚠ {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block font-mono text-[10px] uppercase tracking-widest text-[#3d444d] mb-1.5">Username</label>
              <input type="text" placeholder="yourhandle" required value={username}
                onChange={(e) => setUsername(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className="block font-mono text-[10px] uppercase tracking-widest text-[#3d444d] mb-1.5">Email</label>
              <input type="email" placeholder="you@example.com" required value={email}
                onChange={(e) => setEmail(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className="block font-mono text-[10px] uppercase tracking-widest text-[#3d444d] mb-1.5">Password</label>
              <input type="password" placeholder="••••••••" required minLength={6} value={password}
                onChange={(e) => setPassword(e.target.value)} className={inputCls} />
            </div>

            <button type="submit" disabled={loading}
              className="relative w-full overflow-hidden bg-[#238636] hover:bg-[#2ea043] disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 text-white font-mono font-semibold text-xs uppercase tracking-wider py-3 rounded-sm !mt-5 hover:shadow-[0_0_24px_rgba(35,134,54,0.3)] group"
            >
          ...
          <p className="text-center font-mono text-[11px] text-[#3d444d] mt-6">
            ...
            <Link href="/signin" className="text-[#58a6ff] hover:text-[#79b8ff] transition-colors">
              Sign in
            </Link>
          </p>
